...
const cancelOrNext = function(session, results, next) {
  if (results && results.response && typeof results.response == 'string' && results.response.toLowerCase().trim() == 'cancel') {
...
const name = [
  (session, results, next) => {
    ...
    if (results && results.response && results.response.toLowerCase().trim() != 'skip') {
...
const content = [
  ...
  (session, results, next) => {
    if (session.dialogData.data && !session.dialogData.data.content && results && results.response && results.response.toLowerCase().trim() != 'skip') {
...
const inReplyTo = [
  ...
  (session, results, next) => {
    if (session.dialogData && session.dialogData.data && session.dialogData.data['in-reply-to']) {
      builder.Prompts.choice(session, 'RSVP', ['not a RSVP', 'yes', 'no', 'maybe', 'interested']);
    } else {
      next();
    }
  },
...
const photo = [
  ...
  (session, results, next) => {
    if (!session.dialogData.data.photo && results && results.response && results.response[0] && results.response[0].contentUrl) {
  ...
  (session, results, next) => {
    ...
    micropub.query('syndicate-to').then((res) => {
      ...
      if (options.length > 0) {
        ...
        console.log('Did not find any sydication options');
      } else {
        next();
      }
    }).catch((err) => {
